...
public class BoardController {
	...
	public String boardRegister(int bcode_number,Model model)
	{
	...
	public String boardRegister(int boardstate, BoardDTO boardDTO, RedirectAttributes rttr, Model model )
	{
	...
	public String listPaging(@RequestParam("bcode_number") int bcode_number, 
		...
		if(session.getAttribute("member") != null)
		{
			...
			mb_number = (String)map.get("mb_number");
		...
		if(bcode_number == 1){
			...
			//사용자 주문리스트
			List<OrderDTO> order_list = orderService.getMyOrder(mb_number);
