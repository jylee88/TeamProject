...
<style>
	...
	<div class="container">
		...
		<h1 style="font-family: Lato, sans-serif; font-weight: 600;">후기 게시판</h1>
		...
		<c:forEach var="boardList" items="${boardList}">
		 	<tr>
			 	...
			 	<!-- <td style="width : 30%;"><a href="BoardDetail?b_number=${boardList.b_number}">${boardList.b_title}</a></td>-->	
		...
		<table class="review_table2">
			<tr>
				...
				<td style="">
					<button class="btn btn-primary" onclick="location.href='${contextPath}/board/list_paging?bcode_number=1'">목록</button>				
				</td>
			</tr>
	  	</table>
		        <ul class="pagination" style="text-align : center;">
		            <c:if test="${pageMaker.prev}">
		                <li><a href="${contextPath}/board/list_paging?page=${pageMaker.startPage - 1}&perPageNum=10&bcode_number=1&searchType=${pageMaker.criteria.searchType}&keyword=${pageMaker.criteria.keyword}">이전</a></li>
		            </c:if>
		            <c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
		                <li <c:out value="${pageMaker.criteria.page == idx ? 'class=active' : ''}"/>>
		                    <a href="${contextPath}/board/list_paging?page=${idx}&perPageNum=10&bcode_number=1&searchType=${pageMaker.criteria.searchType}&keyword=${pageMaker.criteria.keyword}">${idx}</a>
		                </li>
		            </c:forEach>
		            <c:if test="${pageMaker.next && pageMaker.endPage > 0}">
		                <li><a href="${contextPath}/board/list_paging?bcode_number=1&page=${pageMaker.endPage + 1}&perPageNum=10&bcode_number=2&searchType=${pageMaker.criteria.searchType}&keyword=${pageMaker.criteria.keyword}">다음</a></li>
		            </c:if>
		        </ul>
			</div>
